...
import { getHotel, listReviews } from "../catalog";
import { BookingForm } from "../components/BookingForm";
import { LocaleLink } from "../components/LocaleLink";
import { loc, tokyoISO, walk, yen } from "../format";
...
export function HotelDetailPage() {
  ...
  const t = useT("hotels");
  const am = useT("amenities");

  if (!hotel) {
    return <Navigate to={localePath(locale, "/")} replace />;
  }

  const reviews = listReviews("hotel", hotel.id);
  const amenities = hotel.amenities.split(",").filter(Boolean);

  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <LocaleLink to="/hotels" className="text-sm text-shu">
        ← {t("back")}
      </LocaleLink>
      <div className="mt-6 grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <aside className="lg:sticky lg:top-6 lg:self-start">
          ...
          <BookingForm
            kind="hotel"
            itemId={hotel.id}
            defaultCheckIn={params.get("checkIn") ?? tokyoISO(14)}
            defaultCheckOut={params.get("checkOut") ?? tokyoISO(17)}
            defaultGuests={params.get("guests") ?? "2"}
          />
        </aside>
      </div>
    </div>
